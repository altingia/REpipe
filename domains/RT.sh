...
cd $ASSEMBLY
echo "REVERSE TRANSCRIPTASE" > $RESULTS/RT/$1RT.out
...
for x in `cat $RESULTS/$1.lst`
	do
		...
		echo $x | tee -a $RESULTS/RT/$1RT.out
		## extract positive hits
		grep "gnl" RTrpsblast.out > RTall.out
		wc -l RTall.out | tee -a $RESULTS/RT/$1RT.out
		...
		for type in `cat $DOMAIN/RTcdd.lst`
			...
			wc -l $type.lst | tee -a $RESULTS/RT/$1RT.out
			## pull out fasta
			...
			grep ">" $type.clust.out | wc -l | tee -a $RESULTS/RT/$1RT.out
...
for type in `cat $DOMAIN/RTcdd.lst`
	do
	cat */RT/$type.clust.fas > $RESULTS/RT/$type.combined.fas
	echo $type | tee -a $RESULTS/RT/$1RT.out
	grep ">" $RESULTS/RT/$type.combined.fas | wc -l | tee -a $RESULTS/RT/$1RT.out
done

## reformatting output monitoring table and creating summary table
tail +1 $RESULTS/RT/$1RT.out | paste -d " " - -
